            ...
            .then((fieldValidationResult) => {
                return helpers.findUserDocuments(req.body.email)
                    .then((findUserDocuments) => {
                        return helpers.validatePassword(req.body.email,req.body.uuid,req.body.password,findUserDocuments.password)
                    }).then((validatePassword) => {
                        logger.trace("Login successfully",req.body.email);
                        return responseFunction.sendSuccess("Login successfully",res,{token:validatePassword});
                    }).catch((err) => {
                        return responseFunction.sendError(err,res);
                    })
            })
